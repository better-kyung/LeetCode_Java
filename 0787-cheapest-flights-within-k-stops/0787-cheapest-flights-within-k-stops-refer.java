class Solution {
    public int findCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
        ...
        while (!pq.isEmpty()) {
            ...
            //도착지(u)까지의 진행 경로(k_visited) 저장, 따로 저장해두진 않지만
                ...
                if (graph.containsKey(u)) {
                    ...
                    for (Map.Entry<Integer, Integer> v : graph.get(u).entrySet()) {
                        ...
                        if (!visited.containsKey(v.getKey()) || k_visited < visited.get(v.getKey())) {
                            //u지점 까지의 비용(price_u) + u 지점을 출발지로 한 도착지까지의 비용
        ...
        //큐의 모든 값이 비워질 때까지 리턴하지 못했다면 경로가 존재하지 않는다는 뜻이므로 -1 리턴
        return -1;
    }
}